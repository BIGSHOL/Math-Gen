...
**2026-09-12 변경:** 사용자의 도형 포함 요청으로 D8의 그림 생략 정책을 폐기했다. 현재는 편집된 그림을 PNG로 보내고 도우미 1.3.0의 `v2-figures`로 삽입한다. 복원·권한·배포 계약은 [hwp-figure-export.md](docs/hwp-figure-export.md)를 따른다.
...
### 44-12. 2단 정답표 칼럼 침범 + 서술형 구분선 줄바꿈 (사용자 2026-06-24, 2단 전용)

**증상 2건**:
1. **표 단 넘어감**: 빠른정답 격자/전폭행 표가 **본문 폭(148mm)** 으로 생성돼 2단 칼럼(~84mm)을
   넘어 **우측 단 해설을 침범**("표 크기 조절 안 해? 단 넘어가잖아").
2. **서술형 구분선 줄바꿈**: `_ESSAY_SEPARATOR`(대시12+서술형+대시12=29자)가 2단 칼럼 한 줄
   (~21자)을 넘어 **"서술형"이 우측 끝, 트레일링 대시가 다음 줄**로 갈라짐.

**root cause (표) — CRITICAL**: COM `table_begin(line_width=W)` 의 `WidthType=0` 은 *절대폭이
아니라 본문 텍스트 폭 채움* — `line_width` 는 **열 비율만** 결정. 1단은 텍스트폭=본문폭(148mm)
이라 맞지만, 2단은 COM 작성 시점이 1단(148mm)이라 표가 148mm 로 생성된 뒤 `_apply_body_columns`
가 colPr=2 만 바꿔 표 폭은 그대로. 본문 보기 박스는 `_inject_bogi_form` 가 **칼럼폭으로 재주입**
해 맞던 것(§42-8) — COM `table_begin` 표엔 그 보정이 없었음(`_box_width()` 는 84mm 반환하지만
table_begin 이 무시).

**수정** (`hwp_com_writer.py`):
- `_fit_wide_tables_2col(hwpx, margins)` 신규 — 2단에서 칼럼폭 넘는 표를 칼럼폭으로 축소(저장 후
  XML, `_scale_bogi_box_width` 패턴). **표 sz + cellSz 만** 비례 축소(셀 안 수식 객체 sz 제외 —
  분수 크기 보존). bogi(이미 ≤ 칼럼폭)는 무시. `write_exam_to_hwp` 의 `columns==2` 마지막 호출.
- `_ESSAY_SEPARATOR_2COL`(대시6+서술형+대시6) 신설 — `self._columns==2` 면 사용(칼럼 한 줄 맞춤).

검증: 2단 정답표 colCnt=3/1 모두 148mm→82mm(칼럼폭, 우측 단 미침범) + 서술형 구분선 한 줄.
1단 무변경(표=본문폭 정상, 구분선 12대시).
