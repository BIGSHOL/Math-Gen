...
import { PAPER_COLORS, PAPER_FONTS, A4_DIM } from "../paperTokens";
import { BodyContainer } from "../BodyContainer";
import { QuestionNumber } from "../ProblemMeta";
import type { PrintTemplateProps } from "../PrintTemplate.types";

declare const ProblemBody: React.FC<{
  problem: { question: string; choices?: string[]; topic?: string };
  fontSize?: number;
}>;

// `conceptNote` 는 markdown 일 수 있음 — 실제 통합 시 MarkdownRenderer 사용.
declare const MarkdownRenderer: React.FC<{ content: string }>;
...
export function JaseupTemplate({
  ...
  const accent = options.accentColor || PAPER_COLORS.accentGold;
              ...
              <div>
                ...
                >
                  ...
                  <span
                    style={{
                      flex: 1,
                      borderBottom: `1px solid ${PAPER_COLORS.ink30}`,
                      height: 16,
                    }}
                  />
                  <span style={{ fontSize: 10, color: PAPER_COLORS.ink50 }}>
                    ✓ 채점
                  </span>
                  <span
                    style={{
