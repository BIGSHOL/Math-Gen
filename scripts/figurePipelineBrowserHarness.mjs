...
try {
  ...
  const results = await page.evaluate(async () => {
    ...
    let figures=[figure], calls=[], renderFails=false, cancel=false, cancelAtDetect=false;
    ...
    try {
      ...
      assert(result.images[0].originalDataUrl===input.figureCrop && result.images[0].engineSvg===svg,'Original crop and engine SVG are both preserved');
      ...
      assert(calls.filter(c=>c.url==='/api/ai-figure').length===2,'Invalid spec gets exactly one repair attempt');
