...
for (const route of ['ai-ocr','ai-cropdetect','ai-solution','ai-variant','ai-image',
  'ai-exam-analysis','ai-exam-commentary','ai-exam-v4','ai-figure-detect','ai-figure','ai-generate','unknown']) {
  let status = 200;
  const response = {
    status(code: number) { status = code; return this; },
    json(_body: unknown) { return this; },
  } as VercelResponse;
  await handler({ method: 'POST', query: { route }, headers: {}, body: {} } as VercelRequest, response);
