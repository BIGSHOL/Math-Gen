  ...
  try {
    ...
    const response = await anthropic.messages.stream({
      ...
      messages: [{ role: "user", content: [
        ...
        { type: "text", text: `Recreate this isolated figure. Return the JSON spec only. For v2, points, segments, circles, angles, dimensions and labels MUST be named OBJECT maps, never anonymous arrays. Example: segments:{"AB":["A","B"]}, labels:{"A":"A"}. IMPORTANT: dimensions creates dashed measurement bows. Use dimensions ONLY if a measurement bow/arrow exists in the original. For a bare number beside a side, add an unconnected anchor point where the number sits and label it with plain text: labels:{"value3":"3"}. Anchor points have no visible dot; do not connect them. Let the engine place every label: omit dx/dy and position (both pin a label to a single spot, so any overlap with a segment or circle fails the whole figure). Move the anchor point itself if a label needs to sit elsewhere. Keep the diagram's visual proportions, even if the printed values imply a different scale. Use only visible labels from the crop, not labels from the example.${repair}` },
      ] }],
    }).finalMessage();
