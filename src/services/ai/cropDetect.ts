...
- 🚨 **ADJACENT BOXES MUST NOT OVERLAP VERTICALLY (within a column).** Each printed problem occupies a *disjoint* vertical band. Box N's BOTTOM (yMax) must sit ABOVE box N+1's TOP (yMin) — leave a small gap at the blank line between problems; never let consecutive boxes share a y-range. A box's TOP = its own problem-number line, its BOTTOM = its own last printed element — so a box must NEVER reach down into the next problem's number line, nor up into the previous problem's last line. If two problems are tightly stacked, place the boundary at the blank gap between them (prev problem's last printed line = prev yMax; next problem's number line = next yMin), with prev.yMax < next.yMin.
- In "note", briefly state what set the bottom edge (e.g., "⑤ bottom-right", "(8점) line", "last sub-part (2)").

## OUTPUT — pure JSON only:
{"items": [{"number": 8, "type": "choice", "class": "problem", "cropBox": [100, 50, 280, 480], "endMarkerKind": "choice", "note": "⑤ bottom-right"}]}`;
...
/** 재시도해도 소용없는 영구 오류(크레딧/키/권한)면 true (testchange _is_permanent). */
const isPermanentCropErr = (msg: string): boolean =>
  /credit|depleted|billing|prepay|balance|api key|api_key|permission|unauthor|invalid|401|403/i.test(
    msg,
  );

const parseCropItems = (rawJson: string): DetectedCrop[] => {
  const parsed = parseJsonOrThrow<{ items?: DetectedCrop[] }>(stripCodeFences(rawJson));
  return Array.isArray(parsed.items) ? parsed.items : [];
};

/** 컬럼 분할 추정 + 좌우 패딩 적용 (검출 모델 무관 공통). */
const padCropItems = (items: DetectedCrop[]): DetectedCrop[] => {
  const split = estimateColumnSplit(items.map((c) => c.cropBox));
  return items.map((c) => ({ ...c, cropBox: padCropBox(c.cropBox, split) }));
};

/** Gemini 3.5 Flash 크롭 검출 (testchange _detect_with_gemini — MIME-only, temp 0.1, 65536). */
const detectWithGemini = async (
  ...
  if (finishReason === "MAX_TOKENS")
    throw new Error("Gemini 토큰 한도 초과 (페이지 분석 실패) — 손글씨/문항 과다.");
  const rawJson = typeof response.text === "string" ? response.text : "";
  if (!rawJson) throw new Error(`Gemini 빈 응답 (finishReason=${finishReason ?? "unknown"}).`);
...
const detectCropBoxesDirect = async (
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      return padCropItems(await detectWithGemini(pageBase64, signal));
    } catch (err) {
      if ((err as Error).name === "AbortError") throw err;
      geminiErr = err as Error;
      // 영구 오류(크레딧/키)거나 마지막 시도면 즉시 폴백, 아니면 backoff 후 재시도.
      if (isPermanentCropErr(geminiErr.message) || attempt === 2) break;
