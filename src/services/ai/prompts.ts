...
🚨 **trial-and-error / self-correction 흔적 절대 금지 (사용자 보고 13번):**

  잘못된 실제 출력 (절대 emit X):
    "...n=2^a × 3^b. 합 = 7×(1+2+...) = 1960. **잠깐, 선택지가 작으므로 0에 들어갈 수 있는 자연수들의 합을 다시 확인**. ... =1120. 여전히 선택지에 없음. **다시 정리:** ... =280. ❤"

  → 모델이 본인 thinking 을 raw 로 emit. "잠깐", "다시 확인", "재정리", "여전히 ... 없음" 같은 self-correction 표현 통째로 누출.

  올바른 패턴: 잘못 계산한 부분이 있으면 **출력에서 그 과정을 *지우고*** 처음부터 정답까지 직선 식 흐름만. 마치 처음부터 정답으로 향한 듯이.

  금지 표현 (정규식으로 거의 검출 가능):
  - "잠깐", "다시 확인", "재정리", "재검토", "다시 계산", "다시 정리"
  - "선택지가 없으니", "여전히 없음", "어차피"
  - "...인데, 사실은", "...로 보이나, 실제로는"
  - "계산 재확인", "다시 풀어보면"
